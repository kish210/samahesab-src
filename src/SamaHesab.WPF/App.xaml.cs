...
public partial class App : System.Windows.Application
{
    ...
    private async Task StartAppAsync(StartupEventArgs e)
    {
        ...
        _host = Host.CreateDefaultBuilder()
            ...
            .ConfigureServices((ctx, services) =>
            {
                ...
                services.AddTransient<SamaHesab.WPF.ViewModels.Treasury.ReceivePaymentViewModel>();   // دریافت/پرداختِ مستقل
                ...
                services.AddTransient<ViewModels.Inventory.WarehouseOverviewViewModel>();   // UX-INV-1 — نمای انبار
    ...
    private async Task<bool> CheckForUpdateAsync()
    {
        try
        {
            ...
            // U-DESKTOP-WEB-MIGRATE — از v2.9 دیگر نصابِ دسکتاپ منتشر نمی‌شود (فقط Web_Setup)؛
            // دانلود/اجرایِ خودکارِ آن رویِ نصبِ دسکتاپ نادرست است (CLOSEAPPLICATIONS یک برنامهٔ
            // متفاوت را جایگزین می‌کند) — به‌جایش فقط پیامِ آگاهی/پیشنهادِ مهاجرت نشان داده می‌شود،
            // کاربر خودش تصمیم می‌گیرد از مرورگر نصاب را بگیرد.
            if (info.IsWebOnly)
            {
                await dialog.ShowInfoAsync(
                    $"نسخهٔ جدیدِ سما حساب ({info.Tag}) منتشر شده — از این پس فقط به‌صورتِ «کلاینتِ وب» " +
                    "(بدونِ نیازِ نصبِ جداگانه روی هر سیستم، از مرورگر) به‌روزرسانی می‌شود.\n" +
                    "برایِ استفاده از امکاناتِ جدید (شاملِ ویزاردِ راه‌اندازی، قالب‌هایِ چاپ، و بقیهٔ فیچرهایِ اخیر) " +
                    $"نصابِ وب را از {UpdateService.WebDownloadPageUrl} دانلود و رویِ سرور نصب کنید؛ " +
                    "سپس همهٔ کاربران از مرورگر با آدرسِ سرور کار می‌کنند — نیازی به نصبِ دسکتاپِ جدید نیست.",
                    "نسخهٔ وبِ جدید موجود است");
                return false;   // فقط اطلاع‌رسانی — جریانِ عادیِ دسکتاپ ادامه می‌یابد
            }
