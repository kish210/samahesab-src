-- =============================================================================
-- SAMA HESAB ERP — ماژول رستوران (v2)
-- اسکیمای Rst: سالن، میز، سفارش، ردیف سفارش، رسید آشپزخانه
-- این اسکریپت idempotent است و روی پایگاه‌داده‌ی موجود هم قابل اجراست.
-- =============================================================================
USE SamaHesab;
GO

IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'Rst')
...
CREATE TABLE Rst.DiningTables (
    ...
    Status         INT NOT NULL DEFAULT 0,   -- 0=Free 1=Occupied 2=Reserved 3=Billing
...
CREATE TABLE Rst.RestaurantOrders (
    ...
    OrderType      INT NOT NULL DEFAULT 0,   -- 0=DineIn 1=Takeaway 2=Delivery
    Status         INT NOT NULL DEFAULT 0,   -- 0=Open 1=Sent 2=Served 3=Settled 4=Cancelled
    TableId        INT NULL,
...
CREATE TABLE Rst.RestaurantOrderItems (
    ...
    Status          INT NOT NULL DEFAULT 0,   -- 0=Pending 1=InKitchen 2=Preparing 3=Ready 4=Served 5=Cancelled
...
CREATE TABLE Rst.KitchenTickets (
    ...
    Status       INT NOT NULL DEFAULT 0,   -- 0=New 1=Preparing 2=Ready 3=Completed
...
-- ─── دادهٔ اولیه: یک سالن پیش‌فرض با ۶ میز (اگر هیچ سالنی نبود) ─────────────────
...
PRINT N'ماژول رستوران (اسکیمای Rst) با موفقیت ساخته شد.';
GO
