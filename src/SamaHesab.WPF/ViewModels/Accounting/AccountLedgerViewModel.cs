...
public partial class AccountLedgerViewModel : BaseViewModel
{
    ...
    public override async Task LoadAsync()
    {
        ...
        {
            Accounts.Clear();
            foreach (var a in await _mediator.Send(new GetAccountsQuery(LeafOnly: true))) Accounts.Add(a);
    ...
    {
        if (SelectedAccountId <= 0) { await _dialogService.ShowWarningAsync("یک حساب را انتخاب کنید."); return; }
        await ExecuteAsync(async () =>
        {
            var res = await _mediator.Send(new GetAccountLedgerQuery(SelectedAccountId, FromDate, ToDate));
            Rows.Clear();
