...
public partial class VoucherListViewModel : BaseViewModel, SamaHesab.WPF.Services.ISupportsNew
{
    ...
    public VoucherListViewModel(
        ...
        ApiClient api) : base(dialogService, navigationService)
    {
        _mediator = mediator;
    ...
    private async Task LoadPreviewAsync(int voucherId)
    {
        try
        {
            ...
            var lines = !string.IsNullOrWhiteSpace(_api.BaseUrl)
                ? (await _api.GetVoucherPreviewAsync(voucherId)).Select(l => new VoucherPreviewLine(l.AccountName, l.Debit, l.Credit)).ToList()
                : (await _mediator.Send(new SamaHesab.Application.Accounting.Queries.GetVoucherPreviewQuery(voucherId)))
                    .Select(l => new VoucherPreviewLine(l.AccountName, l.Debit, l.Credit)).ToList();
            PreviewLines.Clear();
            foreach (var l in lines) PreviewLines.Add(l);
