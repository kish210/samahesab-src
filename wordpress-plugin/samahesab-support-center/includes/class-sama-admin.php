...
class SamaHesab_Admin {
    ...
    public function download_page() {
        if ( isset( $_POST['samahesab_save_dl'] ) && check_admin_referer( 'samahesab_dl' ) ) {
            update_option( 'samahesab_manifest_url', esc_url_raw( wp_unslash( $_POST['manifest_url'] ?? '' ) ) );
        ...
        if ( isset( $_POST['samahesab_clear_cache'] ) && check_admin_referer( 'samahesab_dl' ) ) {
            ...
            echo '<div class="notice notice-success"><p>کَشِ «آخرین نسخه» پاک شد؛ دفعهٔ بعد زنده از kishwifi.com/download خوانده می‌شود.</p></div>';
        }
        $manifest = get_option( 'samahesab_manifest_url', 'https://kishwifi.com/download/version.json' );
        $version  = get_option( 'samahesab_version', '2.5.0' );
        $url      = get_option( 'samahesab_download_url', 'https://kishwifi.com/download/' );
        ?>
        <div class="wrap" dir="rtl" style="font-family:Tahoma,sans-serif">
            <h1>دانلودِ نرم‌افزار (بخشِ سایت)</h1>
            <p><strong>لینکِ دانلود هوشمند است:</strong> شورت‌کدِ <code>[samahesab_product]</code> به‌صورتِ خودکار آخرین نسخه/نصاب را از <code>version.json</code>ِ رویِ <code>kishwifi.com/download/</code> می‌خوانَد و ۱ ساعت کَش می‌کند. کافی است نصابِ تازه + <code>version.json</code>ِ به‌روزشده را با cPanel/FTP روی <code>kishwifi.com/download/</code> آپلود کنی — سایت بدونِ هیچ تغییری به‌روز می‌شود.</p>
            <p>مقادیرِ «نسخه/لینکِ دستی» فقط <strong>fallback</strong> هستند (اگر manifest در دسترس نباشد).</p>
            <form method="post">
                <?php wp_nonce_field( 'samahesab_dl' ); ?>
                <table class="form-table">
                    <tr><th>آدرسِ manifest (version.json)</th><td><input name="manifest_url" type="url" class="regular-text" value="<?php echo esc_attr( $manifest ); ?>" placeholder="https://kishwifi.com/download/version.json"></td></tr>
                ...
                <p>
                    ...
                    <button class="button" name="samahesab_clear_cache" value="1">تازه‌سازیِ آخرین نسخه از kishwifi.com/download</button>
                </p>
            </form>
            <hr>
