...
/// <summary>فاز ۱۲ (پولیش) — گزارشِ گردشِ موجودی (Inventory Turnover) + روزِ ماندگاری + خروجی.</summary>
public partial class InventoryTurnoverViewModel : BaseViewModel
{
    private readonly IMediator _mediator;
    private readonly IPersianCalendarService _calendar;
    private readonly IPdfService _pdf;

    public ObservableCollection<TurnoverRow> Rows { get; } = new();
    ...
    public InventoryTurnoverViewModel(IMediator mediator, IPersianCalendarService calendar, IPdfService pdf,
    ...
    private async Task RunAsync()
    {
        ...
        {
            var rows = await _mediator.Send(new GetInventoryTurnoverQuery(FromDate, ToDate));
            Rows.Clear();
            foreach (var r in rows) Rows.Add(r);
            TotalInventoryValue = rows.Sum(r => r.InventoryValue);
            TotalCogs = rows.Sum(r => r.Cogs);
        }, "در حال محاسبهٔ گردشِ موجودی...");
    }

    private ReportTable BuildTable()
    {
        string N(decimal d) => d.ToString("N0");
        var headers = new[] { "کد", "نام کالا", "ارزشِ موجودی", "بهای فروش‌رفته (COGS)", "نرخِ گردش", "روزِ ماندگاری" };
        var rows = Rows.Select(r => new[] { r.Code, r.Name, N(r.InventoryValue), N(r.Cogs), r.Ratio.ToString("0.##"), r.DaysDisplay }).ToList();
        rows.Add(new[] { "", "جمعِ کل", N(TotalInventoryValue), N(TotalCogs), "", "" });
        return new ReportTable($"گردشِ موجودی ({FromDate} تا {ToDate})", headers, rows);
    }
    ...
    private static string SaveBytes(string ext, byte[] content)
    {
        ...
        var path = System.IO.Path.Combine(dir, $"گردش_موجودی_{System.DateTime.Now:yyyyMMdd_HHmmss}.{ext}");
