# سامانهٔ جامع «سما حساب» — نسخهٔ ۲.۳.۰
...
## ⬇️ دانلود (نسخهٔ ۲.۳.۰)

**نصابِ کاملِ خودکفا** (شاملِ .NET runtime — نیازی به پیش‌نیاز ندارد):

### 👉 [دانلودِ `SamaHesab_Setup_v2.3.0.exe` (~۱۰۸ مگابایت)](https://github.com/kish210/SamaHesab/releases/download/v2.3.0/SamaHesab_Setup_v2.3.0.exe)

> **تازه در ۲.۳.۰:** **ساختِ خودکارِ پایگاه‌داده در اولین اجرا** — دیگر نیازی به اجرای دستیِ اسکریپت‌های `database/` نیست؛ برنامه خودش پایگاه‌داده + جدول‌ها + دادهٔ پایه (نمودار حساب‌ها، شرکت/شعبه/سالِ مالیِ پیش‌فرض، کاربرِ admin) را می‌سازد و سپس مهاجرت‌های افزایشی را اعمال می‌کند. (رفعِ مشکلِ «نصاب پایگاه‌داده را نمی‌سازد» و «فرمِ اطلاعاتِ پایه کار نمی‌کند».)
>
> _نسخهٔ ۲.۲.۰:_ PDFِ بومیِ فارسی + QRِ تصویری · سخت‌سازیِ مارکت‌پلیسِ قالب · ویزاردِ راه‌اندازی + ورودِ اکسل · لایسنسِ آفلاینِ RSA + Windows Service · ۹ ایندکسِ کارایی · صفحه‌بندیِ فهرست‌ها.
...
> **یا** برای نصبِ تک‌سیستمی (سرور+کلاینت روی یک دستگاه): **`SamaHesab_Setup_v2.3.0.exe`**.
...
dotnet test  tests/SamaHesab.Tests            # ~۳۱۱ تست
...
ISCC installer\SamaHesab_Setup.iss    # -> installer/Output/SamaHesab_Setup_v2.3.0.exe
```
