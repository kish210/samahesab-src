=== SamaHesab Support Center ===
Contributors: samarayanehkish
Tags: support, helpdesk, tickets, bug-report, erp
Requires at least: 5.8
Tested up to: 6.5
Requires PHP: 7.4
Stable tag: 1.4.11
License: GPLv2 or later

مرکزِ پشتیبانی و بازخوردِ سما حساب برای وردپرس — دریافتِ گزارشِ باگ/درخواستِ قابلیت/تیکت از نصب‌های ERP،
دانشنامه، یادداشت‌های نسخه و پرتالِ مشتری.

== Description ==

این پلاگین، سمتِ سرورِ «مرکزِ پشتیبانیِ» نرم‌افزارِ حسابداریِ سما حساب است و روی سایتِ وردپرسِ
شرکت (kishwifi.com) نصب می‌شود. کلاینتِ ERP از طریقِ REST با این پلاگین گفت‌وگو می‌کند.

* Custom Post Typeها: تیکت، گزارشِ باگ، درخواستِ قابلیت، یادداشتِ نسخه، مقالهٔ دانشنامه.
* REST API امن با «کلید-API» در فضای‌نامِ `samahesab/v1`.
* منوی ادمینِ «SamaHesab» با داشبوردِ آمار + مدیریتِ مشتری/کلید-API.
* پرتالِ مشتری با شورت‌کد.
* اعلانِ ایمیلیِ ثبت/تغییرِ وضعیت.
* امنیت: فقط دادهٔ پشتیبانی/تشخیصی — هیچ دادهٔ مالی.

== مسیرهای REST ==

پایه: `https://SITE/wp-json/samahesab/v1`
احراز: هدرِ `X-SamaHesab-ApiKey` (+ اختیاری `X-SamaHesab-Customer`, `X-SamaHesab-License`).

* `POST /bug`              — ثبتِ گزارشِ باگ → `{ "Id": "<شناسه>" }`
* `POST /feature`          — ثبتِ درخواستِ قابلیت → `{ "Id": "..." }`
* `POST /ticket`           — ثبتِ تیکت → `{ "Id": "..." }`
* `POST /remote`           — ثبتِ نشستِ پشتیبانیِ ریموت (کد + شناسهٔ RustDesk) → `{ "Id": "..." }`
* `GET  /releases`         — فهرستِ یادداشت‌های نسخه
* `GET  /articles?search=` — جست‌وجوی دانشنامه
* `GET  /status/<id>`      — وضعیتِ یک رکورد

== شورت‌کدها ==

* `[samahesab_product]`  — بخشِ عمومیِ «امکانات + دانلودِ» نرم‌افزار (هیرو + گرید + دکمهٔ دانلود).
* `[samahesab_versions]` — فهرستِ همهٔ نسخه‌های منتشرشده + لینکِ دانلودِ هرکدام (از `versions.json`).
* `[samahesab_portal]`   — تیکت‌ها/درخواست‌های مشتریِ واردشده.
* `[samahesab_kb]`       — دانشنامه با جست‌وجو.
* `[samahesab_releases]` — یادداشت‌های نسخه.

== نصب ==

1. پوشهٔ `samahesab-support-center` را در `wp-content/plugins/` بگذارید.
2. از «افزونه‌ها» فعال کنید (CPTها و مسیرها ساخته می‌شوند).
3. به «SamaHesab → مشتریان (کلید-API)» بروید و برای هر نصبِ ERP یک کلید بسازید.
4. کلید/شناسهٔ مشتری/آدرسِ سایت را در «تنظیماتِ پشتیبانیِ» سما حساب وارد کنید.

== Changelog ==

= 1.4.11 =
* رفعِ گزارشِ کاربر: «لینکِ سایت فقط دسکتاپ بود، وب/سرور/کلاینت نبود» — صفحهٔ اصلیِ محصول (`[samahesab_product]`) کنارِ دکمهٔ بزرگِ دانلود (که همچنان نصابِ تک‌سیستمیِ دسکتاپ را پیش‌فرض می‌دهد)، حالا ردیفِ کوچکِ ۴ نوعِ نصاب (وب/سرور/کلاینت/دسکتاپ) را هم نشان می‌دهد — هرکدام دکمهٔ دانلودِ مستقیم یا نشانِ «به‌زودی».

= 1.4.10 =
* بازطراحیِ `[samahesab_versions]`: به‌جایِ لینکِ خام به هر فایل، هر نسخه اکنون ۴ ردیفِ ثابت نشان می‌دهد — «دسکتاپ (تک‌سیستمی)»/«وب»/«سرور (API)»/«کلاینت» — هرکدام یا دکمهٔ دانلود دارد یا نشانِ خاکستریِ «به‌زودی» (طبقِ درخواستِ کاربر برای دیدنِ صریحِ هر ۴ نوعِ نصاب کنارِ هم).

= 1.4.9 =
* شورت‌کدِ تازهٔ `[samahesab_versions]` — «همهٔ نسخه‌ها»: فهرستِ کاملِ نسخه‌های منتشرشده (نسخه/تاریخِ شمسی/توضیح) با لینکِ دانلودِ هرکدام؛ لینک‌ها فقط از `https://kishwifi.com/download/versions.json` خوانده می‌شوند (نه GitHub) — نسخه‌هایی که فایلِ نصابشان هنوز روی سرورِ خودتان آپلود نشده، نشانِ «به‌زودی» می‌گیرند به‌جایِ لینکِ شکسته.

= 1.4.8 =
* افزودنِ گریدِ امکانات: هتل/اقامتگاه (PMS)، حضوروغیابِ حرفه‌ای با اتصال به دستگاه‌هایِ زدکتکو، پیمانکاری (صورت‌وضعیت+آبشارِ کسورات)، صورتحسابِ الکترونیکیِ مودیان — ماژول‌هایِ تازه‌یی که این نسخه به وبِ سما حساب اضافه شدند.

= 1.4.7 =
* لینکِ دانلود/آپدیت به‌جایِ GitHub Releases API، حالا از `https://kishwifi.com/download/version.json` خوانده می‌شود — نصاب‌ها روی دامنهٔ خودِ شرکت هم میزبانی می‌شوند. تنظیماتِ «SamaHesab ▸ دانلود» با فیلدِ «آدرسِ manifest» جایگزینِ فیلدِ قدیمیِ «مخزنِ گیت‌هاب» شد.

= 1.4.6 =
* رفعِ باگ: فهرستِ گزارش‌های باگ (هم جدولِ «آخرین گزارش‌ها» در داشبورد، هم فهرستِ CPTِ گزارشِ باگ) فقط نوع/عنوان/تاریخ نشان می‌داد — متنِ واقعیِ کاربر (شرحِ مشکل) هیچ‌جای فهرست دیده نمی‌شد و برایِ خواندنش باید هر گزارش را جداگانه باز می‌کردی. حالا ستونِ «شرحِ مشکل» (پیش‌نمایشِ متن) + «شدت» به هر دو جا اضافه شد.

= 1.3.0 =
* پشتیبانیِ ریموت با RustDesk: CPTِ `samahesab_remote` + endpointِ `POST /remote` (کد + شناسهٔ RustDesk + اطلاعاتِ فنی) + ستون‌های فهرستِ ادمین (کد/شناسهٔ RustDesk/مشتری) تا کارشناس درخواست را ببیند و وصل شود.

= 1.2.2 =
* رفعِ ناخوانایی روی تم‌های تیره: بخشِ معرفی حالا **خودبسنده** است — پس‌زمینهٔ سفید، فونتِ صریحِ Vazirmatn، رنگ‌های ثابتِ خوانا (مستقل از رنگ/فونتِ تمِ سایت). حذفِ `color-mix`.

= 1.2.1 =
* مقاوم‌سازی: کَشِ منفیِ ۱۵دقیقه‌ایِ API گیت‌هاب (جلوگیری از بلاک‌شدنِ رندر) + تصویرِ هیرو eager.

= 1.2.0 =
* بازطراحیِ حرفه‌ایِ بخشِ `[samahesab_product]`: هماهنگ با تمِ سایت (توکن‌های رنگِ Elementor + فونتِ ارثی)، هیرو با تصویر، گریدِ امکانات، و **گالریِ اسکرین‌شاتِ واقعیِ نرم‌افزار** (در قابِ پنجره، بزرگ‌نماییِ کلیکی). تصاویر همراهِ پلاگین در `assets/screenshots/`. فایلِ `preview.html` برای پیش‌نمایشِ محلی.

= 1.1.0 =
* بخشِ عمومیِ «امکانات + دانلودِ» نرم‌افزار با شورت‌کدِ `[samahesab_product]` + تنظیماتِ نسخه/لینکِ دانلود (SamaHesab ▸ دانلود).

= 1.0.0 =
* نسخهٔ نخست: CPTها، REST، احرازِ کلید-API، داشبوردِ ادمین، پرتالِ مشتری، اعلانِ ایمیلی.
