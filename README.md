# سامانهٔ جامع «سما حساب» — نسخهٔ ۲.۵.۷
...
> فایل‌های هر release: نصابِ تک‌سیستمی `SamaHesab_<نسخه>_Setup.exe` · نصابِ کلاینت `SamaHesab_Client_<نسخه>.exe` · نصابِ سرور `SamaHesab_Server_<نسخه>.exe`.

> **تازه در ۲.۵.۷:** پولیشِ کاملِ چاپِ فاکتور/قالب‌ها (رفعِ جدولِ به‌هم‌ریخته، ردیف‌های خالی، ارقامِ فارسی، تخفیف/جمع/مبلغ به حروف) · رفعِ **باگِ تایپ‌نشدنِ تکست‌باکس‌ها در فرمِ راه‌اندازیِ اولیه** + مراحلِ دادهٔ پایه (انبار/کالا/خدمات/طرف‌حساب) در ویزارد · فاکتورِ خریدِ کلاسیک.
>
> **تازه در ۲.۵.۵–۲.۵.۶:** **معماریِ کاملِ API-only** (کلاینت فقط از Web API، بدونِ دسترسیِ مستقیمِ DB) · **طرف‌حسابِ یکپارچهٔ چندنقشه** (`Crm.Parties` — ادغامِ مشتری/تأمین‌کننده در یک جدول) · **مرکزِ پشتیبانیِ مشتری** (گزارشِ باگ/تیکت/درخواستِ قابلیت/دانشنامه/عیب‌یابی/ریموت + پلاگینِ وردپرس).
>
> **تازه در ۲.۵.۰:** سخت‌سازیِ سرورِ Web API — **بوت‌استرپِ خودکارِ پایگاه‌داده در استارت‌آپِ سرور** (سرورِ تازه دیگر به DBِ از پیش‌ساخته نیاز ندارد)، endpointِ `/health`، خطای یکدستِ `ProblemDetails` (RFC 7807) · **جداسازیِ امنیتیِ چندشعبه** برای فاکتورِ فروش/خرید و انبار (هر شعبه فقط دادهٔ خودش را می‌بیند) · پایدارسازیِ **به‌روزرسانیِ خودکار از GitHub** (نسخهٔ جدید هنگامِ اجرا به کاربر اعلام و با تأیید نصب می‌شود).
>
> **تازه در ۲.۴.۰ (آمادگیِ تجاری):** مجموعهٔ کاملِ گزارش‌های مدیریتی — **ماندهٔ سنی‌شده**، **خلاصهٔ مالیاتِ ارزش‌افزوده**، **دفترِ روزنامه**، **سود و زیانِ کالا**، **کالای راکد/کم‌گردش**، **تحلیلِ ABC**، **گردشِ موجودی** (همه با خروجیِ اکسل/PDF/چاپ) · «مبلغِ سند به حروف» · **قفلِ دورهٔ مالیِ بسته** در همهٔ مسیرهای ثبت/تغییرِ سند · **کاملیِ حسابرسی** · سیاستِ رمز · زمان‌بندِ خودکارِ بکاپ · اعتبارسنجیِ هویتِ مالیاتی.
>
...
### ماژول‌های اختیاری
- صندوق فروشگاهی **POS** (لمسی، شیفت/Z-X، Hold/Recall) · **رستوران** (میز/گارسون/آشپزخانه) · CRM · **مرکزِ پشتیبانیِ مشتری** (گزارشِ باگ/تیکت/دانشنامه/عیب‌یابی/ریموت + پلاگینِ وردپرس).
...
| دفترِ روزنامه | سود و زیانِ کالا |
|---|---|
| ![روزنامه](screenshot/daybook.png) | ![سود کالا](screenshot/product_profit.png) |

| کالای راکد / کم‌گردش | تحلیلِ ABC کالا |
|---|---|
| ![راکد](screenshot/dead_stock.png) | ![ABC](screenshot/abc_analysis.png) |

| گردشِ موجودی (Inventory Turnover) |
|---|
| ![گردش موجودی](screenshot/inventory_turnover.png) |
...
dotnet test  tests/SamaHesab.Tests            # ~۴۰۵ تست
...
ISCC installer\SamaHesab_Setup.iss    # -> installer/Output/SamaHesab_Setup_v2.5.0.exe
```
